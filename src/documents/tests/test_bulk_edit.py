...
class TestPDFActions(DirectoriesMixin, TestCase):
    ...
    @mock.patch("documents.tasks.bulk_update_documents.delay")
    @mock.patch("documents.tasks.update_document_archive_file.delay")
    def test_rotate_without_archive(self, mock_update_document, mock_update_documents):
        """
        GIVEN:
        - Existing documents
        WHEN:
        - Rotate action is called with 2 documents, one doesn't have an archive file
        THEN:
        - Rotate action should be called once
        """
        doc_ids = [self.doc2.id, self.doc3.id]

        with self.assertLogs("paperless.bulk_edit", level="DEBUG") as cm:
            rotate(doc_ids, 90)

            error_str = cm.output[1]
            expected_str = (
                "Document 3 does not have an archive version, skipping rotation"
            )
            self.assertIn(expected_str, error_str)
    ...
    def test_rotate_with_error(
        ...
        with self.assertLogs("paperless.bulk_edit", level="ERROR") as cm:
            rotate(doc_ids, 90)
            error_str = cm.output[0]
            expected_str = "Error rotating document 2"
            self.assertIn(expected_str, error_str)
